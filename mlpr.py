...
def build_confusion_matrix(annotations):
    ...
    for doc_index, doc_data in enumerate(annotations):
        ...
        predicted_roles = determine_roles_with_openai(doc_data)
        for participant in participants:
            inn = participant.get("inn")
            true_role = participant.get("role")
            if not inn or not true_role:
                continue
            predicted_role = predicted_roles.get(inn,"иное")
            y_true.append(true_role.strip().lower())
            y_pred.append(predicted_role.strip().lower())
    if not y_true:
